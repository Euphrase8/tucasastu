    ...
    const fetchChaplains = async () => {
      ...
      try {
        const response = await fetch(
          "https://api.tucasastu.com/api/chaplaincy-leaders"
        );
        if (!response.ok) throw new Error("Failed to fetch chaplains");
        const data: Leader[] = await response.json();
        setChaplains(
          data.filter((leader) => leader.Title?.toLowerCase() === "chaplain")
        );
      } catch (error) {
      ...
      <div className="container">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
            Our dedicated chaplains provide spiritual guidance, pastoral care,
            and ministerial support to TUCASA STU members across all regions of
            Tanzania.
          </p>
        </div>
              ...
              >
                <div className="relative w-full h-64 overflow-hidden rounded-xl">
                  ...
                  <div className="absolute top-4 left-4">
                    <Badge
                      variant="secondary"
                      className="bg-gold/90 text-gold-foreground"
                    >
                      Chaplain
                    </Badge>
                  </div>
                ...
                <CardContent className="p-4">
                  ...
                  {chaplain.Contact && (
                    <Button
                      ...
                      onClick={() =>
                        (window.location.href = `tel:${chaplain.Contact}`)
                      }
                    >
                      Contact Chaplain
                    </Button>
        ...
        <div className="text-center mt-12">
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl p-8 shadow-card inline-block">
            ...
            <p className="text-muted-foreground mb-6 max-w-2xl mx-auto">
              Discover our full leadership team including national executives,
              zonal presidents, federation leaders, and branch coordinators
              serving across Tanzania.
            </p>
            <Button
              size="lg"
